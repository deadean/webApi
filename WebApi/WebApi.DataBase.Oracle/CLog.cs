//-------------------------------------------------------------------
// <copyright file="CLog.cs" company="Lizenza Development Ltd.">
...
namespace WebApi.DataBase.Oracle
{
	...
	public static class CLog
	{
		...
		static CLog()
		{
			...
			try
			{
				...
				if (name.ToLower().Contains("adaica.exe") || name.ToLower().Contains("adaica.vshost.exe"))
				{
					...
					Log("START LOG: " + System.Windows.Application.ResourceAssembly.FullName);
					System.Windows.Application.Current.Exit += new ExitEventHandler(LogEnd);
					isStarted = true;
				}
			}
			catch (Exception /*exc*/)
		...
		/// <summary>
		/// End logging by adding las record.
		/// </summary>
		/// <param name="sender">The sender.</param>
		/// <param name="e">The <see cref="System.Windows.ExitEventArgs"/> instance containing the event data.</param>
		static void LogEnd(object sender, ExitEventArgs e)
		{
			try
			{
				Log("STOP LOG: " + System.Windows.Application.ResourceAssembly.ManifestModule.Name);
			}
			catch (Exception exc)
			{
				System.Diagnostics.Debug.Print(exc.ToString());
		...
		public static bool SaveBuffer()
		{
			...
			{
#if DEBUG
				MessageBox.Show("SaveBuffer : " + exc.Message, "Logging.", MessageBoxButton.OK, MessageBoxImage.Error);
#endif
				return false;
			}
		}
		...
		static void LogInThreadFromTempMessageList(object messageIn)
		{
			...
			{
#if DEBUG
				MessageBox.Show("Exception  Message:" + e.Message, "Logging.", MessageBoxButton.OK, MessageBoxImage.Error);
