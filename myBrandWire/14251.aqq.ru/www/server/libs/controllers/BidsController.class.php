...
class BidsController extends Controller
{
    public static function init() 
    {
        ...
        self::$methods_params = array(
            ...
            'deleteNew' => array('id'),
            'setStatusPayed' => array('id'),
            'setStatusEdit' => array('id')
        );
    }
    ...
    public static function approvalPassed()
    {
        ...
        if ($bidInfo['status'] != 'editPublishedNews' && $bidInfo['status'] != 'spell_checked')
            return 'Wrong status';
    ...
    public static function deleteNew()
    {
        ...
        $bidId = self::$data['id'];
        $result = BidsModel::deleteNew($bidId);
        
        self::$is_request_ok = true;
        return 1;
    }

    public static function setStatusPayed()
    {
        // Is moderator exists
        $moderatorInfo = ModeratorsModel::getModeratorById(Config::$moderatorId);
        if (!$moderatorInfo)
        ...
        $bidId = self::$data['id'];
        $result = BidsModel::setStatusPayed($bidId);

        self::$is_request_ok = true;
        return 1;
    }

    public static function setStatusEdit()
    {
        // Is moderator exists
        $moderatorInfo = ModeratorsModel::getModeratorById(Config::$moderatorId);
        if (!$moderatorInfo)
        ...
        $bidId = self::$data['id'];
        $result = BidsModel::setStatusEdit($bidId);
